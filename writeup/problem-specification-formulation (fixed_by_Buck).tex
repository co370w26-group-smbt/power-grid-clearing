...
In this project, we focus first on the Day-Ahead (DA) market clearing process, then the clearing of derivative contracts called FTRs. The DA market clearing process involves the grid operator collecting bids from generators (to sell) and loads (to buy) electricity from pre-specified settlement points (pricing nodes) in the power grid for the next 24 hours. In this market, virtual traders also purchase power allocation to be sold later in the real-time market, which settles every 5 minutes in real-time. We decided that this application is out of scope for this assignment and instead we only focus on DA settlement for power and FTRs.
...
\textbf{This proposal is structed into 4 sections.}

The first section (3-9) outline the mathematical intuition behind the dynamics of the market. In power markets, transmission is in real-time and cannot be easily directed through specific power lines, and instead follow fundemental laws of physics and must not be overloaded, or otherwise risk catastrophic failure. Therefore we calculate shift factors to account for physical electricity spread, and introduce safety tests which are done by RTOs during the electricity market clearing process to ensure grid stability and an efficient, competitive market. 

In the second section (12) we motivate intuition by going through a simple worked example using an step-by-step approach which helps lead into the next section where we introduce the LP for systematically solving these optimization problems.

Sections 13, 14 outline the LP formulations for both the DA and FTR clearance processes.

Finally, sections 15 and 16 outline our group's next steps an extensions.
...
Note the RTO balances the grid by settling the auction, such that willing buyers buy the most electricity at the best prices. Then the grid provides a feasible generation/consumption schedule to cleared participants.
...
The objective maximizes social welfare: total willingness-to-pay of cleared load minus total generation cost.
